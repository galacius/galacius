...
### Fix (2026-09-22, commit a8db19d): tray portal overlapped `AppFooter`

`UnifiedTrayShell`'s `DrawerPortal`/`DrawerViewport` had no explicit `container`, so Base UI portaled it to `document.body` and its `fixed inset-0` positioning spanned the whole browser window — overlapping `AppFooter.tsx` (a flex sibling of `MainLayout`, not a descendant of it, so `MainLayout`'s own bounds never constrained the tray). Fixed by giving `MainLayout` a `trayContainerRef` (`useRef<HTMLDivElement>`) attached to its own root div via `MainLayoutProvider`'s new `containerRef` prop (`className` also gained `relative` so the absolutely-positioned viewport has something to anchor to), threading it down through `UnifiedTrayOutlet` to `UnifiedTrayShell`, which passes it as `<DrawerPortal container={containerRef}>` and adds `absolute` to `DrawerViewport`'s className. Any future portal-based overlay that needs to stay within `MainLayout`'s bounds rather than the full viewport should reuse this same `trayContainerRef`/`containerRef` plumbing rather than re-deriving it.
